...
function wrapLines(words, maxLen) {
    const lines = [];
    let currentLine = [];
    let currentLen = 0;

    for (let word of words) {
        // Calculate the length if we add this word (with a '-' if not first word)
        let additionalLen = word.length + (currentLine.length > 0 ? 1 : 0);

        if (currentLen + additionalLen > maxLen) {
